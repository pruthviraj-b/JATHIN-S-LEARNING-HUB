        ...
        <ProtectedRoute requiredRole="STUDENT">
            <StudentLayout>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 15, marginBottom: 30 }}>
                    <div style={{ background: '#09090B', padding: 20, borderRadius: 12, border: '1px solid #27272A' }}>
                        <div style={{ fontSize: 28, fontWeight: '800', color: 'white' }}>{percentage}%</div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-muted)' }}>Attendance Rate</div>
                    </div>
                    <div style={{ background: '#09090B', padding: 20, borderRadius: 12, border: '1px solid #27272A' }}>
                        <div style={{ fontSize: 28, fontWeight: '800', color: 'white' }}>{present}</div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-muted)' }}>Classes Attended</div>
                    </div>
                    <div style={{ background: '#09090B', padding: 20, borderRadius: 12, border: '1px solid #27272A' }}>
                        <div style={{ fontSize: 28, fontWeight: '800', color: 'white' }}>{total}</div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-muted)' }}>Total Classes</div>
                    </div>
                </div>

                {/* Calendar Controls */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 15, background: '#09090B', padding: 15, borderRadius: 12, border: '1px solid #27272A' }}>
                    <button onClick={prevMonth} style={{ background: '#18181B', border: '1px solid #27272A', color: 'white', borderRadius: 8, padding: 8, cursor: 'pointer' }}><ChevronLeft /></button>
                    <span style={{ fontSize: 18, fontWeight: 700, color: 'white' }}>{monthNames[month]} {year}</span>
                    <button onClick={nextMonth} style={{ background: '#18181B', border: '1px solid #27272A', color: 'white', borderRadius: 8, padding: 8, cursor: 'pointer' }}><ChevronRight /></button>
                </div>

                {/* Calendar Grid */}
                <div style={{ background: '#09090B', borderRadius: 12, overflow: 'hidden', border: '1px solid #27272A' }}>
                    {/* Header Days */}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', background: '#18181B', borderBottom: '1px solid #27272A' }}>
                ...
                <div style={{ display: 'flex', gap: 20, marginTop: 15, fontSize: 13, justifyContent: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)' }}><div style={{ width: 10, height: 10, borderRadius: '50%', background: 'white' }}></div> Present</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)' }}><div style={{ width: 10, height: 10, borderRadius: '50%', background: '#3F3F46' }}></div> Absent</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)' }}><div style={{ width: 10, height: 10, borderRadius: '50%', background: '#18181B', border: '1px solid #27272A' }}></div> No Record</div>
                </div>
                    ...
                    {filteredRecords.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>No records found{selectedDay ? ' for this date' : ''}.</p> : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {filteredRecords.map(a => {
                                ...
                                return (
                                    <div key={a.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#09090B', padding: 15, borderRadius: 10, border: '1px solid #27272A', borderLeft: `4px solid ${isPresent ? 'white' : '#3F3F46'}` }}>
                                        <div>
                                            <div style={{ fontWeight: 600, color: 'white' }}>{new Date(a.date).toLocaleDateString()}</div>
                                        ...
                                        <div>
                                            {isPresent ?
                                                <span style={{ color: 'white', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 5 }}>Present <Check size={16} /></span>
                                                : <span style={{ color: 'var(--text-muted)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 5 }}>Absent <X size={16} /></span>
                                            }
                                        </div>
                                    </div>
                                )
