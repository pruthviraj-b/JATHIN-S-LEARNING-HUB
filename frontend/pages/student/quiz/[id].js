...
export default function QuizInterface() {
        ...
        return (
            <ProtectedRoute requiredRole="STUDENT">
                <div style={{ minHeight: '100vh', background: 'black', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ background: '#09090B', padding: 40, borderRadius: 12, border: '1px solid #27272A', textAlign: 'center', maxWidth: 500, width: '100%' }}>
                        <div style={{ fontSize: 60, marginBottom: 20 }}>🎉</div>
                        <h1 style={{ margin: 0, color: 'white' }}>Quiz Completed!</h1>
                        ...
                        <button onClick={() => router.push('/student/tests')} style={{ padding: '12px 30px', background: 'white', color: 'black', border: 'none', borderRadius: 6, fontSize: 16, cursor: 'pointer', fontWeight: 600 }}> Back to Dashboard</button>
                    </div>
                </div>
            </ProtectedRoute>
        )
    ...
    return (
        <ProtectedRoute requiredRole="STUDENT">
            <div style={{ minHeight: '100vh', background: 'black', padding: '30px 0' }}>
                <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 20px' }}>

                    {/* Header */}
                    <div style={{ background: '#09090B', padding: 24, borderRadius: 12, marginBottom: 20, border: '1px solid #27272A', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <h1 style={{ margin: 0, fontSize: 22, color: 'white' }}>{test.title}</h1>
                        ...
                        <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: 20, fontWeight: '600', color: 'white' }}>--:--</div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gap: 20 }}>
                        {test.questions.map((q, index) => (
                            <div key={q.id} style={{ background: '#09090B', padding: 24, borderRadius: 12, border: '1px solid #27272A' }}>
                                <div style={{ marginBottom: 15, fontSize: 16, fontWeight: 600, color: 'white' }}>
                                    ...
                                    {q.options.map(opt => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                style={{
                                                    padding: '12px 15px',
                                                    border: isSelected ? '1px solid white' : '1px solid #27272A',
                                                    borderRadius: 8,
                                                    cursor: 'pointer',
                                            ...
                                            >
                                                <div style={{
                                                    ...
                                                    border: isSelected ? '6px solid white' : '2px solid #3F3F46',
                    ...
                    <div style={{ marginTop: 30, textAlign: 'center' }}>
                        <button
                            ...
                            style={{
                                padding: '15px 40px',
                                background: Object.keys(answers).length < test.questions.length ? '#3F3F46' : 'white',
                                color: Object.keys(answers).length < test.questions.length ? 'var(--text-muted)' : 'black',
                                border: 'none',
                                borderRadius: 8,
                                fontSize: 18,
                                fontWeight: 'bold',
